...
func (ps FileSender) addHTMLHeader(content string, m mail.Message) string {
	header := fmt.Sprintf(htmlHeaderTmpl, html.EscapeString(m.From), strings.Join(m.CC, ","), strings.Join(m.Bcc, ","), strings.Join(m.To, ","), html.EscapeString(m.Subject))
